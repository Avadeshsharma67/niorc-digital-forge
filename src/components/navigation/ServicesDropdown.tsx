...
const ServicesDropdown = ({ servicesOpen, setServicesOpen, onServiceClick }: ServicesDropdownProps) => {
  ...
  return (
    <div className="relative group">
      ...
      <div 
        className={`absolute top-full left-0 mt-3 w-80 bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-200/50 py-6 px-4 transition-all duration-500 transform-gpu ${
          servicesOpen 
      ...
      >
        <div className="space-y-4">
          <div>
            <h3 className="text-sm font-bold text-gray-900 mb-3 flex items-center animate-fade-in">
            ...
            {highDemandServices.map((service, index) => (
              <Link
                ...
                className={`block px-3 py-2 text-sm font-medium rounded-lg transition-all duration-300 transform hover:scale-105 animate-fade-in ${
